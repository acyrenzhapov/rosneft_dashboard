...
# Reference (1) : https://pytorch.org/docs/stable/_modules/torchvision/models/
# resnet.html#resnet18
...
class ResNet(nn.Module):
    ...
    def __init__(
        self, input_channels, block, layers, num_classes=1,
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, channels, stride=1, dilation=1):
        ...
        self.conv2 = nn.Conv3d(
            channels, channels, kernel_size=3, stride=stride, padding=dilation,
            dilation=dilation, bias=False
            )
        self.bn2 = nn.BatchNorm3d(channels)

        self.conv3 = nn.Conv3d(
            channels, out_channels, kernel_size=1, bias=False
            )
...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers, input_channels):
        ...
        self.layer5 = make_layer(
            BasicBlock, in_channels=256, channels=512, num_blocks=num_blocks,
            stride=1, dilation=2
            )
...
class ResNet_BasicBlock_OS8(nn.Module):
    def __init__(self, num_layers, input_channels):
        ...
        self.layer4 = make_layer(
            BasicBlock, in_channels=128, channels=256,
            num_blocks=num_blocks_layer_4, stride=1, dilation=2
            )

        self.layer5 = make_layer(
            BasicBlock, in_channels=256, channels=512,
...
def ResNet101_OS16(input_channels):
    return ResNet_Bottleneck_OS16(
        num_layers=101, input_channels=input_channels
        )


def ResNet152_OS16(input_channels):
    return ResNet_Bottleneck_OS16(
        num_layers=152, input_channels=input_channels
        )
